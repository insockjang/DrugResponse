...
id_headerLayer <- "syn1584019"
layer_header <- loadEntity(id_headerLayer)
header1 <- layer_header$objects$MMSE



featureData <- createAggregateFeatureDataSet(list(expr = res2, prot = protein2))

data_drug <- as.numeric(as.matrix(header1$MMSE))
names(data_drug)<-as.character(header1$SAMPLE.ID)
...
save(resultsLasso,resultsENet,file = "~/Results/AD/MMSE/AD/bs100Results_exprProtein2.Rdata")
...
save(resultsLasso,resultsENet,file = "~/Results/AD/MMSE/AD/bs100Results_exprProtein1.Rdata")
