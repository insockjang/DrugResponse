...
for(kk in 1:ncol(dataSets_ccle$responseData)){  
  ...
  resultsScale<-crossValidatePredictiveModel1(filteredFeatureDataScaled,filteredResponseDataScaled, model = mySplsModel$new(), eta = etas, K = Ks, nfolds = 5)
  resultsRank<-crossValidatePredictiveModel1(filteredFeatureDataRank,filteredResponseDataRank, model = mySplsModel$new(), eta = etas, K = Ks, nfolds = 5)
  
  filename <- paste("SPLS/cvDrug_",kk,"_Spls.Rdata",sep ="")  
...
filename1 <- "SPLS/cv5performance.Rdata"
