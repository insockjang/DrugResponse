...
for(kk in 1:ncol(pData(adf_drug))){
  ...
  resultsCat<-crossValidatePredictiveModel_categorical(filteredFeatureDataScaled, filteredResponseDataScaled, model = myRandomForestModel$new(), numFolds = 5,ntree = 100)
  filename = paste("~/COMPBIO/trunk/users/jang/drugResponse_woMut/CCLE/randomForest/ROCR_median_",kk,".Rdata",sep = "")
