...
for(kk in 1:ncol(dataSets_ccle$responseData)){  
  ...
  resultsLasso2<-bootstrapPredictiveModel(filteredFeatureDataScaled, filteredResponseDataScaled, model = myEnetModel$new(), numBootstrap=100, alpha=1, lambda = lambdas)
  ...
  Results[[kk]]<-list(resultsENet = resultsENet,resultsLasso=resultsLasso,resultsRidge=resultsRidge)
}
