...
for(kk in 1:ncol(dataSets_ccle$responseData)){  
  ...
  resultsRidge<-bootstrapPredictiveModel(filteredFeatureDataScaled, filteredResponseDataScaled, model = myEnetModel$new(), numBootstrap=100, alpha=10^-8, lambda = lambdas)
  filename1 <- paste("bootstrapENet2/cvDrug_",kk,".Rdata",sep ="")  
  filename2 <- paste("bootstrapLasso2/cvDrug_",kk,".Rdata",sep ="")  
  filename3 <- paste("bootstrapRidge2/cvDrug_",kk,".Rdata",sep ="")  
  save(resultsENet,file = filename1)
  save(resultsLasso,file = filename2)
  save(resultsRidge,file = filename3)
