...
for(kk in 1:ncol(dataSets_ccle$responseData)){  
  ...
  resultsScale<-crossValidatePredictiveModel1(filteredFeatureDataScaled, filteredResponseDataScaled, model = CaretModel$new(modelType = "svmLinear"), trControl = myTrControl)
  resultsRank<-crossValidatePredictiveModel1(filteredFeatureDataRank, filteredResponseDataRank, model = CaretModel$new(modelType = "svmLinear"), trControl = myTrControl)
  
  filename <- paste("NPSVM/cvDrug_",kk,".Rdata",sep ="")  
...
filename1 <- "NPSVM/cv5performance.Rdata"
