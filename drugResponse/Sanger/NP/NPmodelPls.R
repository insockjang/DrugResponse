...
for(kk in 23:ncol(dataSets_ccle$responseData)){  
  ...
  resultsScale<-crossValidatePredictiveModel1(filteredFeatureDataScaled, filteredResponseDataScaled, model = CaretModel$new(modelType = "pls"), trControl = myTrControl,tuneGrid = tuneGrid)
  resultsRank<-crossValidatePredictiveModel1(filteredFeatureDataRank, filteredResponseDataRank, model = CaretModel$new(modelType = "pls"), trControl = myTrControl,tuneGrid = tuneGrid)
  
  filename <- paste("NPPLS/cvDrug_",kk,"_Pls.Rdata",sep ="")  
...
filename1 <- "NPPLS/cv5performance.Rdata"
