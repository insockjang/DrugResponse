...
AUC<-matrix(0,nrow = 130,ncol=7)
for(kk in 1:130){
  filename1 <-paste("/Volumes/ijang/COMPBIO/trunk/users/jang/drugResponse/Sanger/PCR/ROCR_",kk,".Rdata",sep ="")
  load(filename1)
  AUC[kk,1]<-resultsCat$erAUC@y.values[[1]]
  filename2 <-paste("/Volumes/ijang/COMPBIO/trunk/users/jang/drugResponse/Sanger/PLS/ROCR_",kk,".Rdata",sep ="")
  load(filename2)
  AUC[kk,2]<-resultsCat$erAUC@y.values[[1]]
  filename3 <-paste("/Volumes/ijang/COMPBIO/trunk/users/jang/drugResponse/Sanger/SVM/ROCR_",kk,".Rdata",sep ="")
  load(filename3)
  AUC[kk,3]<-resultsCat$erAUC@y.values[[1]]
  filename4 <-paste("/Volumes/ijang/COMPBIO/trunk/users/jang/drugResponse/Sanger/randomForest/ntree100/ROCR_",kk,".Rdata",sep ="")
  load(filename4)
  AUC[kk,4]<-resultsCat$erAUC@y.values[[1]]
  
  filename5 <-paste("/Volumes/ijang/COMPBIO/trunk/users/jang/drugResponse/Sanger/Lasso2/ROCR_",kk,".Rdata",sep ="")
  load(filename5)  
  AUC[kk,5]<-resultsCat$erAUC@y.values[[1]]  
  filename6 <-paste("/Volumes/ijang/COMPBIO/trunk/users/jang/drugResponse/Sanger/Ridge2/ROCR_",kk,".Rdata",sep ="")
  load(filename6)
  AUC[kk,6]<-resultsCat$erAUC@y.values[[1]]
  filename7 <-paste("/Volumes/ijang/COMPBIO/trunk/users/jang/drugResponse/Sanger/ENet2/ROCR_",kk,".Rdata",sep ="")
  load(filename7)
  AUC[kk,7]<-resultsCat$erAUC@y.values[[1]]
  
}

colnames(AUC)<-c("PCR","PLS","SVM","categoricalRandomForest","Lasso","Ridge","ENet")
...
png("predictivePerformanceAnalysis/Sanger_AUC.png",width = 1800, height = 900)
plot_colors <- c("black","blue","red","darkred","magenta","cyan","green")
