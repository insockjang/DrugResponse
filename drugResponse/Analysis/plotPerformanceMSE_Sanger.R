...
for(kk in 1:130){
  filename = paste("/Volumes/ijang/COMPBIO/trunk/users/jang/drugResponse/Sanger/PCR/cvDrug_",kk,".Rdata",sep ="")
  load(filename)
  MSE[kk,1]<-mse(resultsScale)
  COR[kk,1]<-correlation(resultsScale)
  
  filename = paste("/Volumes/ijang/COMPBIO/trunk/users/jang/drugResponse/Sanger/PLS/cvDrug_",kk,".Rdata",sep ="")
  load(filename)
  ...
  filename = paste("/Volumes/ijang/COMPBIO/trunk/users/jang/drugResponse/Sanger/Lasso2/cvDrug_",kk,".Rdata",sep ="")
  load(filename)
  MSE[kk,4]<-mse(resultsScale)
  COR[kk,4]<-correlation(resultsScale)
  
  filename = paste("/Volumes/ijang/COMPBIO/trunk/users/jang/drugResponse/Sanger/Ridge2/cvDrug_",kk,".Rdata",sep ="")
  load(filename)
  MSE[kk,5]<-mse(resultsScale)
  COR[kk,5]<-correlation(resultsScale)
  
  filename = paste("/Volumes/ijang/COMPBIO/trunk/users/jang/drugResponse/Sanger/ENet2/cvDrug_",kk,".Rdata",sep ="")
  load(filename)
  MSE[kk,6]<-mse(resultsScale)
  COR[kk,6]<-correlation(resultsScale)
}

colnames(MSE)<-c("PCR","PLS","SVM","Lasso","Ridge","ENet")
rownames(MSE)<-drugName

colnames(COR)<-c("PCR","PLS","SVM","Lasso","Ridge","ENet")
...
a<-sort(MSE[,6],decreasing =T,index.return=T)
...
png("predictivePerformanceAnalysis/Sanger_MSE.png",width = 1800, height = 900)
plot_colors <- c("black","blue","red","magenta","cyan","green")
...
png("predictivePerformanceAnalysis/Sanger_COR.png",width = 1800, height = 900)
