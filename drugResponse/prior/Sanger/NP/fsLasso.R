...
#   resultsR<-crossValidatePredictiveModel1(Data4$featureData, Data4$responseData, model = myEnetModel$new(), alpha=alphas, lambda = lambdas, nfolds = 5)
  ...
  lassoCGC <- myEnetModel$new()
  lassoCGC$customTrain(Data1$featureData, Data1$responseData, alpha = alphas, lambda = lambdas, nfolds = 5)
  resultsCGC<-lassoCGC$getCoefficients()
  
  lassoTF <- myEnetModel$new()
  lassoTF$customTrain(Data2$featureData, Data2$responseData, alpha = alphas, lambda = lambdas, nfolds = 5)
  resultsTF<-lassoTF$getCoefficients()
  
  lassoM <- myEnetModel$new()
  lassoM$customTrain(Data3$featureData, Data3$responseData, alpha = alphas, lambda = lambdas, nfolds = 5)
  resultsM<-lassoM$getCoefficients()
  
  lassoR <- myEnetModel$new()
  lassoR$customTrain(Data4$featureData, Data4$responseData, alpha = alphas, lambda = lambdas, nfolds = 5)
  resultsR<-lassoR$getCoefficients()
  
  filename <- paste("Lasso2/fsDrug_",kk,".Rdata",sep ="")  
