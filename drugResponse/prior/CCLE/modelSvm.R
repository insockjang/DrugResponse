...
for(kk in 1:ncol(dataSet1$responseData)){  
  ...
  resultsCGC<-crossValidatePredictiveModel1(Data1$featureData, Data1$responseData, model = CaretModel$new(modelType = "svmLinear"), trControl = myTrControl)
  resultsTF<-crossValidatePredictiveModel1(Data2$featureData, Data2$responseData, model = CaretModel$new(modelType = "svmLinear"), trControl = myTrControl)
  resultsM<-crossValidatePredictiveModel1(Data3$featureData, Data3$responseData, model = CaretModel$new(modelType = "svmLinear"), trControl = myTrControl)
  resultsR<-crossValidatePredictiveModel1(Data4$featureData, Data4$responseData, model = CaretModel$new(modelType = "svmLinear"), trControl = myTrControl)
  
  
  filename <- paste("SVM/cvDrug_",kk,".Rdata",sep ="")  
