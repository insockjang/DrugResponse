...
for(kk in 1:ncol(pData(adf_drug))){
  ...
  filename = paste("~/COMPBIO/trunk/users/jang/drugResponse_woExp/Sanger/catLasso2/ROCR_mean_sd_",kk,".Rdata",sep = "")
  save(resultsCat,file = filename)
  
  resultsCat<-crossValidatePredictiveModel_categorical1(filteredFeatureDataScaled, filteredResponseDataScaled, model = myCatEnetModel$new(), alpha=1, lambda = lambdas, thresholdMethod = "mean")  
  filename = paste("~/COMPBIO/trunk/users/jang/drugResponse_woExp/Sanger/catLasso2/ROCR_mean_",kk,".Rdata",sep = "")
  save(resultsCat,file = filename)
  
  resultsCat<-crossValidatePredictiveModel_categorical1(filteredFeatureDataScaled, filteredResponseDataScaled, model = myCatEnetModel$new(), alpha=1, lambda = lambdas, thresholdMethod = "median_mad")  
  filename = paste("~/COMPBIO/trunk/users/jang/drugResponse_woExp/Sanger/catLasso2/ROCR_median_mad_",kk,".Rdata",sep = "")
  save(resultsCat,file = filename)
  
  resultsCat<-crossValidatePredictiveModel_categorical1(filteredFeatureDataScaled, filteredResponseDataScaled, model = myCatEnetModel$new(), alpha=1, lambda = lambdas, thresholdMethod = "median")  
  filename = paste("~/COMPBIO/trunk/users/jang/drugResponse_woExp/Sanger/catLasso2/ROCR_median_",kk,".Rdata",sep = "")
  save(resultsCat,file = filename)
  print(kk)  
}
