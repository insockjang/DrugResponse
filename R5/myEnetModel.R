...
#' @export
require(glmnet)
myEnetModel <- setRefClass(Class = "myEnetModel",
                           ...
                           methods = list(
                             ...
                             customTrain = function(featureData, responseData, alpha = alpha, lambda = lambda, nfolds = 5,...){
                               if(length(alpha)==1){
                                .self$model <- cv.glmnet(featureData,responseData, alpha = alpha, lambda = lambda, nfolds = nfolds,...) 
                                ...
                                .self$model$optParam <- optParam
                                 ...
                                 for(k in 1:length(alpha)){
                                   fit[[k]]<-cv.glmnet(featureData,responseData, alpha = alpha[k], lambda = lambda, nfolds = nfolds,...)
