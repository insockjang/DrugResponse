...
library(standGL)
source("~/COMPBIO/trunk/users/jang/R5/myExpandStandGLModel.R")
...
# copySet and oncomapSet and Sanger drug sensitivity

# expressionSet
id_copyLayer <- "syn269019"     
...
for(kk in 1:ncol(dataSets_ccle$responseData)){  
  ...
  expandStandGLModel<-myExpandStandGLModel$new()
  expandStandGLModel$customTrain(DATA2, filteredResponseDataScaled, nfolds = 10, index = index2, alpha=1, lambda = lambdas)
  
  resultsScale<-crossValidatePredictiveModel1(DATA1, filteredResponseDataScaled, model = myExpandStandGLModel$new(), alpha=1, lambda = lambdas, numFolds = 5,index = index,nfolds = 10)
