...
predicted<-predict(fit,t(as.matrix(test24ld)),type = "response")


AA<-as.matrix(test24ld[,which(is.na(apply(test24ld,2,mean))==0)])
predicted<-predict(fit,t(AA),type = "response")
observed <- as.numeric(test24catBSEP[which(is.na(apply(test24ld,2,mean))==0)])
...
filename = paste("~/COMPBIO/trunk/users/jang/Lara_BSEP/ROCR_categorical_real_24_ld.Rdata",sep = "")
