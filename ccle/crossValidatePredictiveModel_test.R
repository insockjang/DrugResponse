...
crossValidatePredictiveModel_test <- 
  function(featureData, responseData, model, numFolds = 5, trControl = defaultTrainControl(), tuneGrid = NULL, ...){
    ...
    modelResults <- foreach(fold = foldIndices) %dopar% {
      foldModel <- model
      foldModel$customTrain(featureData[-fold,], responseData[-fold], tuneGrid = NULL,...)
      return(foldModel)
    }
