...
pathwayAnalysisBootstrap<-function(mSigDB_annotations,DB,pathwayName,bootstrapFolder,modelName,numCompounds,prior = FALSE,rankTransform = FALSE){
  ...
  for(kk in 1:numCompounds){
    ...
    testSet = names(referenceSet$x[1:100])
    ...
    analyticResult <-foreach (curPathway = allPathways) %dopar%{    
      ...
      if(pathwayAnalysis$fetResult$p.value<0.5){
...
pathwayAnalysisNonbootstrap<-function(mSigDB_annotations,DB,pathwayName,bootstrapFolder,modelName,numCompounds,prior = FALSE,rankTransform = FALSE){
...
# pathwayAnalysisFuctions for bootstrapping and nonbootstrapping
newPathwayAnalysisBootstrap<-function(mSigDB_annotations,DB,pathwayName,bootstrapFolder,modelName,numCompounds,prior = FALSE,rankTransform = FALSE){
  require(graphite)  
  if(is.element(pathwayName,"BIOCARTA")){
    pathwayDB<-biocarta
  }
...
newPathwayAnalysisNonbootstrap<-function(mSigDB_annotations,DB,pathwayName,bootstrapFolder,modelName,numCompounds,prior = FALSE,rankTransform = FALSE){
  require(graphite)  
  if(is.element(pathwayName,"BIOCARTA")){
    pathwayDB<-biocarta
  }
