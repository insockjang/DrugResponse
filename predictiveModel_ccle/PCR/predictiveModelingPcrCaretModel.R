...
for(kk in 1:ncol(dataSets_ccle$responseData)){  
  ...
  tuneGrid =expand.grid(.ncomp = seq(1,50,1))
  
  model = CaretModel$new(modelType = "pcr")
  
  resultsScale<-crossValidatePredictiveModel1(filteredFeatureDataScaled, filteredResponseDataScaled, , tuneGrid=tuneGrid, trControl = myTrControl)
  
  resultsScale<-crossValidatePredictiveModel1(filteredFeatureDataScaled, filteredResponseDataScaled, model = CaretModel$new(modelType = "pcr"), tuneGrid=tuneGrid, trControl = myTrControl)
  filename <- paste("cvDrug_",kk,"_PCR_scale.Rdata",sep ="")  
