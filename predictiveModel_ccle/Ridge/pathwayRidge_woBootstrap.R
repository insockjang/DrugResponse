### Pathway analysis with Ridge : bootstrapping is useless, so ridge has no code for bootstrapping
...
for(kk in 1:ncol(dataSets_ccle$responseData)){    
  ...
  finalRidgeModel <- myEnetModel$new()      
  finalRidgeModel$customTrain(filteredFeatureDataScaled, filteredResponseDataScaled, alpha = alphas, lambda = lambdas,nfolds = 5)
  referenceSet <- finalRidgeModel$getCoefficients()
  ...
  filename <- paste("ridgeFeatureAnalysisWithoutBootstrap_",kk,".Rdata",sep = "")
