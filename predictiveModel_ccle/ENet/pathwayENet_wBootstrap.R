### Pathway analysis with Enet with preselected drugs
...
for(kk in 1:ncol(dataSets_ccle$responseData)){    
  ...
  for(boot in boots){
    myENetModel <- myEnetModel$new()    
    
    myENetModel$customTrain(filteredFeatureDataScaled[boot,], filteredResponseDataScaled[boot], alpha = alphas, lambda = lambdas,nfolds = 5)
    
    coeff <- as.vector(myENetModel$getCoefficients() !=0)
    
    coeffENet <- cbind(coeffENet, coeff)
  ...
  penaltyFactor[which(apply(coeffENet,1,sum) >= threshold)] <- 0
  
  finalENetModel <- myEnetModel$new()      
  finalENetModel$customTrain(filteredFeatureDataScaled, filteredResponseDataScaled, alpha = alphas, lambda = lambdas,nfolds = 5)
  referenceSet <- finalENetModel$getCoefficients()
  ...
  filename <- paste("eNetFeatureAnalysisWithBootstrap_",kk,".Rdata",sep ="")
