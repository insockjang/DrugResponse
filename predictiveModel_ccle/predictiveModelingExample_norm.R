...
responseData_filtered_norm <- scale(as.numeric(responseData_filtered))
...
predictiveModel_myBlasso$train(t(featureData_filtered_norm), t(responseData_filtered_norm),mcmcNum = 5, burnIn = 1)
...
predictiveModel_eNet$train(t(featureData_filtered_norm), t(responseData_filtered_norm), tuneGrid=createENetTuneGrid(alphas=1))
...
predictiveModel_ICR$train(t(featureData_filtered_norm), t(responseData_filtered_norm), tuneGrid=expand.grid(.n.comp = c(1,2,3,4)))
...
predictiveModel_PCR$train(t(featureData_filtered_norm), t(responseData_filtered_norm), tuneGrid=expand.grid(.ncomp = c(1,2,3,4)))
...
predictiveModel_PLS$train(t(featureData_filtered_norm), t(responseData_filtered_norm), tuneGrid=expand.grid(.ncomp = c(1,2,3,4)))
...
predictiveModel_SVM$train(t(featureData_filtered_norm), t(responseData_filtered_norm))
...
predictiveModel_LS$train(t(featureData_filtered_norm), t(responseData_filtered_norm))
...
predictiveModel_myLs$train(t(featureData_filtered_norm), t(responseData_filtered_norm))
...
predictiveModel_myEnet$train(t(featureData_filtered_norm), t(responseData_filtered_norm), tuneGrid=createENetTuneGrid(alphas=1))
...
predictiveModel_myIcr$train(t(featureData_filtered_norm), t(responseData_filtered_norm), tuneGrid=expand.grid(.n.comp = c(1,2,3,4)))
...
predictiveModel_myPcr$train(t(featureData_filtered_norm), t(responseData_filtered_norm), tuneGrid=expand.grid(.ncomp = c(1,2,3,4)))
...
predictiveModel_myPls$train(t(featureData_filtered_norm), t(responseData_filtered_norm), tuneGrid=expand.grid(.ncomp = c(1,2,3,4)))
...
predictiveModel_mySvm$train(t(featureData_filtered_norm), t(responseData_filtered_norm),tuneGrid=expand.grid(.C = c(0.001,seq(0.05,0.95,by = 0.05),0.999)))
...
# Elastic Net
cvResults_eNet <- crossValidatePredictiveModel(t(featureData_filtered_norm), t(responseData_filtered_norm), model=CaretModel$new(modelType="glmnet"), tuneGrid=createENetTuneGrid(alphas=1), numFolds=3)

#cvResults_myEnet <- crossValidatePredictiveModel(t(featureData_filtered_norm), t(responseData_filtered_norm), model=myEnetModel$new(), tuneGrid=createENetTuneGrid(alphas=1), numFolds=3)

# Least Square
cvResults_Ls <- crossValidatePredictiveModel(t(featureData_filtered_norm), t(responseData_filtered_norm), model=CaretModel$new(modelType="lm"), numFolds=3)

#cvResults_myLs <- crossValidatePredictiveModel(t(featureData_filtered_norm), t(responseData_filtered_norm), model=myLsModel$new(), numFolds=3)

# Partial Least Square
cvResults_Pls <- crossValidatePredictiveModel(t(featureData_filtered_norm), t(responseData_filtered_norm), model=CaretModel$new(modelType="pls"),tuneGrid=expand.grid(.ncomp = c(1,2,3,4)), numFolds=3)

#cvResults_myPls <- crossValidatePredictiveModel(t(featureData_filtered_norm), t(responseData_filtered_norm), model=myPlsModel$new(), tuneGrid=expand.grid(.ncomp = c(1,2,3,4)), numFolds=3)

# Support Vector Machine Regression
cvResults_Svm <- crossValidatePredictiveModel(t(featureData_filtered_norm), t(responseData_filtered_norm), model=CaretModel$new(modelType="svmLinear"), tuneGrid=expand.grid(.C = c(0.001,seq(0.05,0.95,by = 0.05),0.999)), numFolds=3)

#cvResults_mySvm <- crossValidatePredictiveModel(t(featureData_filtered_norm), t(responseData_filtered_norm), model=mySvmModel$new(), tuneGrid=expand.grid(.C = c(0.001,seq(0.05,0.95,by = 0.05),0.999)), numFolds=3)

boxplot(cvResults_eNet$getTestError(), cvResults_Ls$getTestError(), cvResults_Pls$getTestError(),cvResults_Svm$getTestError(), log="y", names = c("ENet", "LS","Partial LS","SVM"), ylab="Test Error", col="blue")



## Warning : mcmcNum and burnIn should be specified before running !!!
# Bayesian Lasso
cvResults_myBlasso <- crossValidatePredictiveModel(t(featureData_filtered_norm), t(responseData_filtered_norm), model=myBlassoModel$new(), mcmcNum =5000, burnIn = 1000, numFolds=3)
#cvResults_myBlasso$plotPredAndObs()                                                

# Bayesian Ridge
cvResults_myBridge <- crossValidatePredictiveModel(t(featureData_filtered_norm), t(responseData_filtered_norm), model=myBridgeModel$new(), mcmcNum =5000, burnIn = 1000, numFolds=3)
#cvResults_myBridge$plotPredAndObs()

boxplot(cvResults_eNet$getTestError(), cvResults_Ls$getTestError(), cvResults_Pls$getTestError(),cvResults_Svm$getTestError(), cvResults_myBlasso$getTestError(),cvResults_myBridge$getTestError(), log="y", names = c("ENet", "LS","PLS","SVM","Blasso","Bridge"), ylab="Test Error", col="blue")

boxplot(cvResults_myBlasso$getTestError(),cvResults_myBridge$getTestError(), log="y", names = c("Blasso","Bridge"), ylab="Test Error", col="blue")


### Bootstarpping test
# Elastic Net
source("/home/ijang/COMPBIO/trunk/users/jang/bootstrap/bootstrapPredictiveModel.R")
bbbsResults_eNet <- bootstrapPredictiveModel(t(featureData_filtered_norm), t(responseData_filtered_norm), model=CaretModel$new(modelType="glmnet"), tuneGrid=createENetTuneGrid(alphas=1))
plot(bsResults_eNet)
...
probeIDLayer <- Layer(list(name = "R_Expression_GSK_RMA_HCoV_GeneSymbol", type = "E", parentId = "114505", status="db"))
testLayer <- addObject(probeIDLayer, eSet)
testLayer <- storeEntity(testLayer)

outputFileComp <- 'norm_boxplot6.pdf'

compLayer <- Layer(list(
                             name="Six Results Comparison for PLX4720",
                             type="M", 
                             parentId=propertyValue(dataset, "id")))
compLayer <- addFile(compLayer, outputFileComp)
storeEntity(compLayer)
